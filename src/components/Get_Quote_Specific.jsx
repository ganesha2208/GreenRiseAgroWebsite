...
const GetQuoteSpecific = ({ onClose, product }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const userRequest = {
        phoneNumber,
        productTitle: product?.name || "No product selected",
  ...
  const productDetails = getAllProduct.find(
    (item) => item.title === product.name
  );

  console.log("Product Details gansha:", productDetails);
              ...
              <div>
                {product && (
                  <div className="text-sm text-gray-600 mt-2">
                    ...
                    {getAllProduct
                      .filter((item) => item.title === product.name)
                      .map((item) => (
...
export default GetQuoteSpecific;
